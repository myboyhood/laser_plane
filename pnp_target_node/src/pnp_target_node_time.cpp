...
//        }
        ...
        cv::putText(ir_img_color_show,
                    "target_position_of_drone x : " + Convert(target_position_of_drone.x()) + "    y: " + Convert(target_position_of_drone.y())  + "    z:" + Convert(target_position_of_drone.z()),
                    cv::Point(5, 40), cv::FONT_HERSHEY_SIMPLEX ,0.6,cv::Scalar(0,0,255),1,8,false);
        ...
        if(pnpGoodFlag){
            msg_drone_pos_vision.pose.orientation.z = -target_position_of_drone.z();
        }
        else{
            msg_drone_pos_vision.pose.orientation.z = 0.0;
        }
