...
bool yolo_process(cv::Mat &frame){
    ...
    if(targetDetectFlag){
        ...
        cv::circle(ir_img_color_show,cv::Point(yolo_center.x,yolo_center.y),3,(0,200,200),3);
...
//        ROS_INFO("yolo_area: %f", yolo_area);
        //根据小孔成像原理估计深度
        target_yolo_distance.z = target_width_world * fx_color / sqrt(target_width*target_height) ;
