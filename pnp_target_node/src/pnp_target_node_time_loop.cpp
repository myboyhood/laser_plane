...
cv::Point2i target_left_up_origin;
cv::Point2i target_right_down;
cv::Point2i target_right_down_origin;
...
int structElementSize_loop = 1;
cv::Mat dilatestructElement = getStructuringElement(cv::MORPH_RECT,cv::Size(structElementSize + 1, structElementSize + 1));
cv::Mat dilatestructElement_loop = getStructuringElement(cv::MORPH_RECT,cv::Size(structElementSize_loop + 1, structElementSize_loop + 1));
...
vector<cv::Point2f> pnpImgPts;
vector<uchar> status;
vector<float> err;
//cv::TermCriteria termcrit(cv::TermCriteria::EPS, 50, 0.001);
cv::TermCriteria termcrit(cv::TermCriteria::EPS, 100, 0.001);
cv::Size winSize(25, 25);
...
//    cv::namedWindow("ir_img_color_show");
    ...
    //!循环尝试：二值化threshold 和 膨胀size
    int binary_threshold_loop = 100; //init threshold
    while(ros::ok()){
        ROS_INFO_THROTTLE(1,"in prepare loop");
        ros::spinOnce();
        data = pipe.wait_for_frames();
        ir = data.get_infrared_frame();
        //图片获取异常处理
...
//    threshold(ir_img_crop, ir_binary, 180, 255, cv::THRESH_BINARY);
        threshold(ir_img_crop, ir_binary, binary_threshold_loop, 255, cv::THRESH_BINARY);


        imshow("ir_binary", ir_binary);
        for(int i = 1; i < 6; i++){
            structElementSize_loop = i;
            dilate(ir_binary, ir_dilate, dilatestructElement_loop);
        ...
        drawContours(ir_img_color_show, ir_contours_final, -1, (250, 100, 200), -1);
        cv::imshow("test param windows", ir_img_color_show);
        cv::waitKey(1);
        if(ir_contours_final.size() == 4){
            ROS_WARN("get threshold param and dilate size");
            binary_threshold = binary_threshold_loop;
            structElementSize = structElementSize_loop;
            break;
        }

        }
        binary_threshold_loop += 10;
        if(binary_threshold_loop > 230){
            break;
        }
    ...
    while(ros::ok()){
        ...
        cout << "binary_threshold" << binary_threshold << endl;
        cout << "structElementSize" << structElementSize << endl;
        ros::spinOnce();
        data = pipe.wait_for_frames();
        ir = data.get_infrared_frame();
        //图片获取异常处理
...
//        drone_pos_vision.y() = - target_position_of_world.y();
        drone_pos_vision.y() = - target_position_of_drone.y();
...
//        }
        ...
        cv::putText(ir_img_color_show,
                    "drone_pos_visionaa x : " + Convert(drone_pos_vision.x()) + "    y: " + Convert(drone_pos_vision.y())  + "    z:" + Convert(drone_pos_vision.z()),
...
void target_corner_cb(const geometry_msgs::QuaternionStamped::ConstPtr& msg)
{
    ...
    {   wait_count --;
        ...
        target_left_up_origin.x = int(max(0,int(msg->quaternion.x)));
...
//    marker_struct.emplace_back(cv::Point3f(0.250,0.205,0.0));
    ...
    marker_struct.emplace_back(cv::Point3f(-0.205,-0.250,0.0));
    marker_struct.emplace_back(cv::Point3f(-0.205,0.250,0.0));
    marker_struct.emplace_back(cv::Point3f(0.205,-0.250,0.0));
    marker_struct.emplace_back(cv::Point3f(0.205,0.250,0.0));
...
//                         0, termcrit, 0, 0.001);
    ...
    if (prevImgPts.size() == nextImgPts.size()) {
        int win_width = 5;
        pnpImgPts = nextImgPts;
        for(int i = 0; i < nextImgPts.size(); i++){
            if(nextImgPts[i].x < 11 || nextImgPts[i].x > 628 || nextImgPts[i].y < 11 || nextImgPts[i].y > 468){
                ROS_ERROR("points edge: failed.");
                return false;
            }
            cv::Point2f filterPts;
            int count_filter = 0;
            for(int j = - win_width; j < win_width; j++){
                for(int k = - win_width; k < win_width; k++){
                    int index_x = j + nextImgPts[i].x;
                    int index_y = k + nextImgPts[i].y;
                    if(frame.at<uchar>(index_y,index_x) > 240){
                        filterPts += cv::Point2f(index_x,index_y);
                        count_filter++;
                    }
                }
            }
            filterPts.x = filterPts.x/count_filter;
            filterPts.y = filterPts.y/count_filter;
            nextImgPts[i].x = (int)filterPts.x;
            nextImgPts[i].y = (int)filterPts.y;
        }
        circle(ir_img_color_show, nextImgPts[i], 4, cv::Scalar(0,255,0), 3, 8);
        outputPointsVector = nextImgPts;
//        outputPointsVector = pnpImgPts;
        swap(nextImg, prevImg);
...
//    threshold(ir_img_crop, ir_binary, 180, 255, cv::THRESH_BINARY);
    ...
    drawContours(ir_img_color_show, ir_contours_final, -1, (250, 100, 0), -1);
...
//        if(abs(abs(imgPoints[0].x -imgPoints[2].x) - abs(imgPoints[1].x - imgPoints[3].x)) < 50){
            ...
            if(abs(msg_target_pose_from_img.pose.position.x) > 2 || abs(msg_target_pose_from_img.pose.position.y) > 2 || abs(msg_target_pose_from_img.pose.position.z) > 6 || abs(msg_target_pose_from_img.pose.position.z) < 1){

                cv::putText(ir_img_color_show,
                        to_string(msg_target_pose_from_img.pose.position.z),
...
//        ROS_INFO("yolo_area: %f", yolo_area);
        //根据小孔成像原理估计深度
        target_yolo_distance.z = scale_size * target_width_world * fx_color / sqrt(target_width*target_height) ;
        target_yolo_distance.x = target_yolo_distance.z * (yolo_center.x-cx) / fx_color;
        target_yolo_distance.y = target_yolo_distance.z * (yolo_center.y-cy) / fy_color;
