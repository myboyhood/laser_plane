...
class XbeeUAV:
    def __init__(self, id=""):
        ...
        self.car_px4_local_pos_from_planehome =Odometry()
        ...
        # port
        self.ser = Serial("/dev/xbee_base", 19200, timeout=0.03)
        ...
        self.car_px4_local_from_planehome_pub = rospy.Publisher('/car_pos_vel_topic', Odometry, queue_size=1)
        # lon_x lat_y ratio
    ...
    def recv_pose(self):
        ...
        while not rospy.is_shutdown():
            ...
            print(len(data))
            if len(data) == 29:
                head1, head2, self._lon, self._lat, self.car_local_x_from_power, self.car_local_y_from_power, self.car_local_vx, self.car_local_vy, end = unpack('@2b6ib', data)
                ...
                self.car_px4_local_pos_from_planehome.header.stamp = rospy.Time.now()
                self.car_px4_local_pos_from_planehome.pose.pose.position.x = self.car_px4_local_pos_from_start.pose.position.x + self.car_to_plane_bias_x;
                self.car_px4_local_pos_from_planehome.pose.pose.position.y = self.car_px4_local_pos_from_start.pose.position.y + self.car_to_plane_bias_y;
                self.car_px4_local_pos_from_planehome.twist.twist.linear.x = self.car_local_vx
                self.car_px4_local_pos_from_planehome.twist.twist.linear.y = self.car_local_vy
                rospy.logwarn('car_px4_local_x_from_planehome: %f', self.car_px4_local_pos_from_planehome.pose.pose.position.x)
                rospy.logwarn('car_px4_local_y_from_planehome: %f', self.car_px4_local_pos_from_planehome.pose.pose.position.y)
            else:
            ...
            self.car_px4_local_from_planehome_pub.publish(self.car_px4_local_pos_from_planehome)
