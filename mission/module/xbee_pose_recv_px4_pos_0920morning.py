...
class XbeeUAV:
    def __init__(self, id=""):
        ...
        self.car_px4_local_setp_follow_pos = PoseStamped()
        self.car_plane_error = PoseStamped()
        ...
        self.plane_local_x = 0
        self.plane_local_y = 0
        ...
        self.car_plane_error_setp_follow_pub = rospy.Publisher('car_plane_error_setp_follow',PoseStamped,queue_size=1)
        self.car_px4_local_setp_follow_pub = rospy.Publisher('/car_px4_local_setp_follow_position', PoseStamped, queue_size=1)
        # lon_x lat_y ratio
    ...
    def plane_local_pos_cb(self,data):
        ...
        self.plane_local_x = self.plane_home_local.pose.pose.position.x
        self.plane_local_y = self.plane_home_local.pose.pose.position.y
        if self.get_home_local and abs(self.plane_home_local.pose.pose.position.x) > 0.001:
            ...
            if self.count >= self.avg_num:
                rospy.loginfo("sum over , avg value")
        ...
        rospy.loginfo("local_sum count = %d", self.count)
    ...
    def recv_pose(self):
        ...
        while not rospy.is_shutdown():
            ...
            print(len(data))
            if len(data) == 29:
                head1, head2, self._lon, self._lat, self.car_local_x, self.car_local_y, self.car_local_vx, self.car_local_vy, end = unpack('@2b6ib', data)
                self.car_local_x = self.car_local_x / 100.0
                self.car_local_y = self.car_local_y / 100.0
                self.car_local_vx = self.car_local_vx / 100.0
                self.car_local_vy = self.car_local_vy / 100.0
                """ acqurie car home bias"""
                if self.get_car_home_bias and abs(self.car_local_x) > 0.1:
                    self.car_home_x = self.car_local_x
                    self.car_home_y = self.car_local_y
                ...
                print("car_local_x", self.car_local_x)
                print("car_local_y", self.car_local_y)
                print("home_lon", self.plane_home_gps_static_lon)
                print("home_lat", self.plane_home_gps_static_lat)
                print("car_home_x:",self.car_home_x)
                print("car_home_y:",self.car_home_y)
                ...
                """  use px4 local pos"""
                self.car_px4_local_pos.pose.position.x = self.car_local_x
                self.car_px4_local_pos.pose.position.y = self.car_local_y
                """ car_px4_local_setp_follow_xy , consider plane and car initial drift"""
                self.car_px4_local_setp_follow_pos.pose.position.x = self.car_local_x + self.car_to_plane_bias_x - self.car_home_x 
                self.car_px4_local_setp_follow_pos.pose.position.y = self.car_local_y + self.car_to_plane_bias_y - self.car_home_y 
                ...
                rospy.loginfo("px4_local_car_x: %f", self.car_local_x)
                rospy.loginfo("px4_local_car_y: %f", self.car_local_y)
                rospy.loginfo("car_px4_local_setp_follow_pos_x: %f", self.car_px4_local_setp_follow_pos.pose.position.x)
                rospy.loginfo("car_px4_local_setp_follow_pos_y: %f", self.car_px4_local_setp_follow_pos.pose.position.y)
                rospy.loginfo("plane_local_x: %f", self.plane_local_x)
                rospy.loginfo("plane_local_y: %f", self.plane_local_y)
                self.car_plane_error.pose.position.x = self.car_px4_local_setp_follow_pos.pose.position.x - self.plane_local_x
                self.car_plane_error.pose.position.y = self.car_px4_local_setp_follow_pos.pose.position.y - self.plane_local_y
                print("car_px4_local_setp_follow_pos_x ", self.car_px4_local_setp_follow_pos.pose.position.x )
                print("car_px4_local_setp_follow_pos_y ", self.car_px4_local_setp_follow_pos.pose.position.y )
            else:
            ...
            self.car_px4_local_setp_follow_pub.publish(self.car_px4_local_setp_follow_pos)
            self.car_plane_error_setp_follow_pub.publish(self.car_plane_error)
            rate.sleep()
