...
class XbeeUAV:
    def __init__(self, id=""):
        ...
        self._lat = 0
        self._sum = 0
        self.car_pos.pose.position.x = 0
        self.car_pos.pose.position.y = 0
        self.yaw_bias = 0.0
    ...
    def recv_pose(self):
        ...
        while not rospy.is_shutdown():
            ...
            print(len(data))
            if data != "":
                head1,head2,self._lon,self._lat,self._sum,end = unpack('@2b3ib', data)
            ...
            rospy.loginfo("car_x: %f", self.car_pos.pose.position.x)
            rospy.loginfo("car_y: %f", self.car_pos.pose.position.y)
